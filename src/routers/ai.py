...
from src.schemas import ConversationHistoryMessage, LLMRequest, InitConverastionRequest, UserMessageRequest, ResetConversationRequest, Profile
...
async def init_conversation_background(request: InitConverastionRequest):
    ...
    try:
        ...
        ai_response = await ask(LLMRequest(client_phone=request.client_phone, topic=request.topic))
        ...
        if ai_response.get('rag_context'):
            await hs.add_message_to_conversation_history(ConversationHistoryMessage(client_phone=request.client_phone, message=ai_response['rag_context'], role='user'))
        ...
        if ai_response.get('tool_calls'):
            for tool_call in ai_response['tool_calls']:
                ...
                await hs.add_message_to_conversation_history(ConversationHistoryMessage(client_phone=request.client_phone, message=tool_message, role='tool'))
        
        # Add tool responses to history if present
        if ai_response.get('tool_responses'):
            for tool_response in ai_response['tool_responses']:
                await hs.add_message_to_conversation_history(ConversationHistoryMessage(client_phone=request.client_phone, message=tool_response, role='tool'))

       
       
        # Add assistant response to history
        await hs.add_message_to_conversation_history(ConversationHistoryMessage(client_phone=request.client_phone, message=ai_response['content'], role='assistant'))
...
async def ask(request: LLMRequest) -> Dict[str, Any]:
    ...
    system_instructions = await history_service.get_instructions(topic=request.topic if request.topic is not None else "Продать") 
    ...
    if not request.prompt and system_instructions:
        if isinstance(system_instructions, dict) and 'prompt' in system_instructions:
            instructions_context = [{'role': 'user', 'content': system_instructions['prompt']}]
